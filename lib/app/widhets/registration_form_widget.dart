...
class NewAuthScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => const Scaffold(body: Center(child: Text('Auth Screen')));
}
...
class _RegistrationFormState extends State<RegistrationForm>
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      final Map<String, dynamic> farmerData = {
        'user_id': int.parse(userId), // Ensure user_id is included
        ...
        'village': _villageController.text.trim(), // Include new field
      ...
      // Call the API
      // NOTE: Replace this IP with your actual server address
      final url = 'http://192.168.43.43:5000/api/farmers/add_farmer_info'; 
      print('Sending request to: $url');
      print('Request body: ${jsonEncode(farmerData)}');
      ...
      if (mounted) {
        if (response.statusCode >= 200 && response.statusCode < 300) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text('Farmer information submitted successfully!'),
              backgroundColor: Colors.green[700],
            ),
          );
        } else {
          // Handle error response
          String errorMessage = 'Failed to submit farmer information';
          try {
  ...
  Widget _buildMultiSelect(
      String title, List<String> allItems, List<String> selectedItems) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 16),
      child: Column(
            ...
            slivers: [
              ...
              SliverToBoxAdapter(
                child: FadeTransition(
                  ...
                  _buildSectionTitle(getText('personalInfo'), Icons.person,
                      Colors.blue.shade700),
                  _buildTextFormField(
                      getText('farmerName'), _farmerNameController),
                  _buildTextFormField(getText('age'), _ageController,
                      keyboardType: TextInputType.number),
                  _buildDropdown(
                      getText('gender'),
                      selectedGender,
                      [getText('male'), getText('female'), getText('other')],
                      (v) => setState(() => selectedGender = v)),
                  _buildTextFormField(getText('contact'), _contactController,
                      keyboardType: TextInputType.phone, validator: (v) {
                    if (v == null || v.isEmpty) return getText('requiredField');
                    if (v.length != 10) return getText('invalidContact');
                    return null;
                  }),
                  _buildTextFormField(getText('aadharCard'), _aadharController,
                      keyboardType: TextInputType.number, validator: (v) {
                    if (v == null || v.isEmpty) return getText('requiredField');
                    if (v.length != 12) return getText('invalidAadhar');
                    return null;
                  }),
                  TextFormField(
                  ...
                  _buildTextFormField(getText('address'), _addressController,
                      maxLines: 3),
                  
                  // New fields: Village, City, District, State
                  _buildTextFormField(getText('village'), _villageController),
                  _buildTextFormField(getText('city'), _cityController),
                  _buildTextFormField(getText('district'), _districtController),
                  _buildTextFormField(getText('state'), _stateController),
                  
                  _buildTextFormField(getText('email'), _emailController,
                      isRequired: false,
                  ...
                  // --- Farm Information ---
                  _buildSectionTitle(
                      getText('farmInfo'), Icons.eco, Colors.green.shade800),
                  _buildTextFormField(getText('landArea'), _landAreaController,
                      keyboardType:
                  ...
                  _buildMultiSelect(getText('irrigationSources'),
                      getList('irrigationOptions'), selectedIrrigationSources),
                  // --- Farming Details ---
                  _buildSectionTitle(getText('farmingDetails'),
                      Icons.agriculture, Colors.orange.shade800),
                  _buildMultiSelect(
                      getText('cropsGrown'), getList('crops'), selectedCrops),
                  _buildDropdown(
                  ...
                  _buildTextFormField(
                      getText('cropDescription'), _cropDescriptionController,
                      isRequired: false, maxLines: 3),
                  // --- Other Information ---
                  _buildSectionTitle(
                      getText('otherInfo'), Icons.info, Colors.purple.shade700),
                  _buildTextFormField(
                      getText('occupation'), _occupationController),
                  _buildTextFormField(
                      getText('familyInfo'), _familyInfoController,
                      isRequired: false, maxLines: 2),
                  _buildDropdown(
                      getText('workType'),
                      selectedWorkType,
                      getList('workTypes'),
                      ...
                      isRequired: false),
                  _buildMultiSelect(getText('cattleBreeder'),
                      getList('cattleOptions'), selectedCattle),
                  _buildTextFormField(
                      getText('poultryInfo'), _poultryInfoController,
                      isRequired: false),
                  _buildDropdown(
                      ...
                      isRequired: false),
                  _buildTextFormField(getText('feedback'), _feedbackController,
